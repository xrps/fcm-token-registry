...
test('allows checking if a token is already registered in a given group', (t) => {
  const storage = createMongoDbEntryStorage({ db: t.context.db });
  const collection = t.context.db.collection(DEFAULT_COLLECTION_NAME);
  const entry = {
    token: 'asdf1234token',
  ...
  return collection
    .insertOne(entry)
    .then(() => Promise.all([storage.exists(entry), storage.exists(nonExistentEntry)]))
...
test('returns saved registry after the registry is saved', (t) => {
  const storage = createMongoDbEntryStorage({ db: t.context.db });
  const expectedEntry = {
    token: 'asdf1234token',
    groupId: 'someone',
  ...
  return storage
    .saveEntry(expectedEntry)
    .then(savedEntry => t.deepEqual(savedEntry, expectedEntry));
});
